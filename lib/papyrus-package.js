...
const PeerConnectionLayer = require('./client/peer-connection-layer.js');
const PortalBindingManager = require('./client/portal-binding-manager.js');
...
class PapyrusPackage {
   ...
   * @param {Object} portalBindingManager
   */
  constructor(options) {
    const {
      workspace, notificationManager, clipboard, portalBindingManager,
      tooltipManager, commandRegistry
    } = options;
  ...
  async sharePortal() {
    ...
    this.showPopover();
    const portalBinding =
      await this.portalBindingManager.createAndInitialiseHostPortalBinding();
