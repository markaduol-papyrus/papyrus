...
class PortalListComponent {
   ...
   */
  async _renderGuestPortalBindingComponents() {
    const manager = this.props.portalBindingManager;
    const guestPortalBindings = await manager.getGuestPortalBindings();

    const portalBindingComponents = await guestPortalBindings.forEach(
      (portalBinding) => {
