...
const log = require('loglevel').getLogger('connection-layer');
...
// Setup logging
log.setLevel(config.logLevels.connectionLayer);

// Constants
const _MAX_SEND_MESSAGE_RETRIES = 10;
const _DEFAULT_HOSTNAME = '127.0.0.1';

// Threshold for max. number of bytes queued to be sent over the data channel
// at any given time.
const _DEFAULT_DATA_CHANNEL_BUFFER_THRESHOLD = 4096;
...
class ConnectionLayer {
  ...
  /**
   * Expected parameters
   * @param {MessageQueue} downstreamMessageQueue Message queue from which
   * messages will be consumed
   * @param {MessageQueue} upstreamMessageQueue Message queue to which messages
   * should be published
  ...
  constructor(props) {
    ...
    this.hostname = props.hostname || _DEFAULT_HOSTNAME;
    this.dataChannelBufferFullThresold = _DEFAULT_DATA_CHANNEL_BUFFER_THRESHOLD;
    ...
    this.messageQueueSubscriptions = new CompositeDisposable();
  }

  getDownstreamMessageQueue() {
    if (!this.downstreamMessageQueue) {
      const logObj = {downstreamMessageQueue: this.downstreamMessageQueue};
      log.warn('Null or undefined incoming message queue: ', logObj);
    }
    return this.downstreamMessageQueue;
  }

  getUpstreamMessageQueue() {
    if (!this.upstreamMessageQueue) {
      const logObj = {upstreamMessageQueue: this.upstreamMessageQueue};
      log.warn('Null or undefined outgoing message queue: ', logObj);
    }
    return this.upstreamMessageQueue;
  }
  ...
  _handleMessageFromServer(message) {
    ...
    switch (msg.type) {
      ...
      case ASSIGN_PEER_ID: {
        ...
        this.upstreamMessageQueue.publish(message);
        break;
      }
   ...
   */
  activateDownstreamMessageQueueListeners() {
    log.debug('Activating listeners for downstream message queue.');

    this.messageQueueSubscriptions.add(
      this.downstreamMessageQueue.onDidPublishMessage(message => {
  ...
  _handleIncomingMessage(message) {
    let logObj = {message: message};
    log.debug('Handling message from downstream message queue: ', logObj);
  ...
  deactivateMessageQueueListeners() {
    log.debug('Deactivating message queue listeners.');

    this.messageQueueSubscriptions.dispose();
  }
  ...
  async sendMessageToPeer(message, targetPeerId, retries=0) {
    ...
    switch (dataChannel.readyState) {
      ...
      case 'open': {
        ...
        this._sendMessagesToPeer(dataChannel, sendQueue);
  ...
  async _sendMessagesToPeer(dataChannel, messages, i=0) {
    const sendMessages = () => {
      while (i < messages.length) {
        if (dataChannel.bufferedAmount > this.dataChannelBufferFullThresold) {
          dataChannel.addEventListener('bufferedamountlow', listener);
        } else {
          dataChannel.send(serializeMessage(messages[i]));
          i += 1;
        }
      }
    };

    // Listen for when number of messages in data channel's buffer is low
    const listener = () => {
      dataChannel.removeEventListener('bufferedamountlow', listener);
      sendMessages();
    };

    // Send messages over the given data channel
    sendMessages();
  }
