  ...
  activateListeners() {
    ...
    this.subscriptions.add(
      this.incomingMessageQueue.onDidPublishMessage(message => {
        ...
        if (header.portalHostPeerId === localPeerId) {
          if (header.targetPeerId !== localPeerId) {
            let errorMessage = 'Message\'s portal host peer ID equals local ';
            errorMessage += 'peer ID but message\'s target peer ID does not ';
            errorMessage += 'equal local peer ID: ' + JSON.stringify(message);
