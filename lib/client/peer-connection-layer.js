...
class PeerConnectionLayer {
  constructor() {
    ...
    this.dataChannelInitPromises = new Map();
  }
   ...
   */
  _handleServerConnectionOpen(eventInfo) {
    log.debug('Handling open server connection: ', {eventInfo: eventInfo});
  ...
  async connectToPeer(targetPeerId) {
    ...
    this.rtcPeerConnectionInitPromises.set(targetPeerId, promise1);
    this.dataChannelInitPromises.set(targetPeerId, promise2);
   ...
   */
  _createDataChannel(targetPeerId, resolveDataChannelInitPromise) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let statusChangeHandler = (eventInfo) => {
        log.debug('RTCDataChannel status change: ', {eventInfo: eventInfo});

        switch (eventInfo.readyState) {
          case 'connecting':
            break;
          case 'open':
   ...
   */
  _createRtcPeerConnection(targetPeerId, resolvePeerConnInitPromise) {
    ...
    return new Promise(async (resolve) => {
      ...
      peerConnection.onconnectionstatechange = (eventInfo) => {
        const logObj = {eventInfo: eventInfo};
        log.debug('RTCPeerConnection connection state change: ', logObj);

        switch (eventInfo.connectionState) {
          case 'new':
            break;
          case 'connecting':
            break;
      ...
      peerConnection.oniceconnectionstatechange = (eventInfo) => {
        const logObj = {eventInfo: eventInfo};
      ...
      peerConnection.onsignalingstatechange = (eventInfo) => {
        const logObj = {eventInfo: eventInfo};
      ...
      peerConnection.ondatachannel = (eventInfo) => {
        let info = 'RTCPeerConnection received RTCDataChannel endpoint ';
        info += 'from remote peer: ';
        log.debug(info, {eventInfo: eventInfo});

        const dataChannel = eventInfo.channel;
      ...
      peerConnection.onicegatheringstatechange = (eventInfo) => {
        const logObj = {eventInfo: eventInfo};
        log.debug(
      ...
      peerConnection.onpeeridentity = (eventInfo) => {

      };

      // Fires when a MediaStream is removed from this connection
      peerConnection.onremovestream = (eventInfo) => {

      };

      // Fires when a track has been added to the connection
      //peerConnection.ontrack = (eventInfo) => {
  ...
  async sendMessageToPeer(message, targetPeerId, retries=0) {
    ...
    const dataChannelInitPromise =
      this.dataChannelInitPromises.get(targetPeerId);
