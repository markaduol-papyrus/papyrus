...
class PeerConnectionLayer {
  constructor(controller) {
    // Attach controller from higher layer
  ...
  _createRtcPeerConnection(targetPeerId) {
    return new Promise((resolve) => {
      ...
      peerConnection.ondatachannel = (event) => {
        ...
        let handleMessageOverDataChannel = (event) => {
          let message = event.data;
          log('Delivering message: ' + message);
          this.controller.handleDeliveredMessage(message)
        }

        dataChannel.onopen = statusChangeHandler;
        dataChannel.onclose = statusChangeHandler;
        dataChannel.onmessage = handleMessageOverDataChannel
      };
  ...
  broadcastMessage(message) {
    this.sendQueues.push(message);

    for (let targetPeerId in this.dataChannels) {
      this.sendMessageToPeer(targetPeerId, message);
    }
  }
