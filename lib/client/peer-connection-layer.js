...
class PeerConnectionLayer {
  ...
  _validateMessageOverDataChannel(message) {
    ...
    if (!message.senderPeerId || message.senderPeerId === this.localPeerId) {
      invalidMessageOverDataChannel = true;
      errorMessage = 'Invalid sender peer ID in message from data channel: ';
    }
    if (!message.targetPeerId && !message.targetPeerIds) {
      invalidMessageOverDataChannel = true;
      errorMessage = 'No target peer ID(s) in message from data channel: ';
    }
    if (message.targetPeerId && message.targetPeerIds) {
      invalidMessageOverDataChannel = true;
      errorMessage = 'Ambigous: Message has both "targetPeerId" and ';
      errorMessage += '"targetPeerIds" keys: ';
    }
    if (message.targetPeerId === message.senderPeerId) {
      invalidMessageOverDataChannel = true;
      errorMessage = 'Target peer ID is equal to sender peer ID: ';
    }
    if (!message.type) {
      invalidMessageOverDataChannel = true;
      errorMessage = 'No message type in message over data channel: ';
    }
  ...
  _handleRTCDataChannelMessageEvent(event) {
    ...
    const msgBody = deserializeMessage(event.data)
    const validMsgBody = this._validateMessageOverDataChannel(msgBody);
