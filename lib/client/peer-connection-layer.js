...
class PeerConnectionLayer {
  ...
  async connectToPeer(targetPeerId) {
    ...
    // Create data channel
    const dataChannel =
      await this._createRtcDataChannel(targetPeerId);
   ...
   * created and stored.
   */
  _createRtcDataChannel(targetPeerId) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const statusChangeHandler = (event) => {
        const logObj = {event: event, readyState: event.readyState};
        log.debug('RTCDataChannel status change: ', logObj);
        ...
        switch (event.readyState) {
          ...
          case 'open':
            ...
            if (!resolveDataChannelInitialised) {
              ...
              log.warn(
                'No initialisation promise for RTCDataChannel: ', logObj
              );
            } else {
              resolveDataChannelInitialised();
            }
            break;
          case 'closing':
          case 'closed':
