...
class HostPortalBinding {
  ...
  onReceivedJoinPortalRequest(callback) {
    this.emitter.on('received-join-portal-request', callback);
  }

  onAcceptedJoinPortalRequest(callback) {
    this.emitter.on('accepted-join-portal-request', callback);
  }

  onReceivedLeavePortalRequest(callback) {
    this.emitter.on('received-leave-portal-request', callback);
  }

  onAcceptedLeavePortalRequest(callback) {
    this.emitter.on('accepted-leave-portal-request', callback);
  }
  ...
  onEnqueueMessage(callback) {
    this.emitter.on('enqueue-message', callback);
  }

  onEnqueueMessageBatch(callback) {
    this.emitter.on('enqueue-message-batch', callback);
  }

  onEnqueueMessageBatches(callback) {
    this.emitter.on('enqueue-message-batches', callback);
  }
  ...
  async handleRemoteMessage(msg) {
    ...
    switch (msg.type) {
      ...
      case LOCAL_PEER_ID: {
        ...
        this.notificationManager.addInfo('Local Peer ID: ' + localPeerId);
        break;
      }
      ...
      case JOIN_PORTAL_REQUEST: {
        ...
        this.emitter.emit('enqueue-message', msg);
        break;
      }
      ...
      case SITE_ID_ACKNOWLEDGEMENT: {
        const {senderPeerId, siteId, senderUsername} = msg;
        await this._registerNewGuestPeer(siteId, senderPeerId, senderUsername);
        ...
        const msg = new MessageBuilder().
                    ...
                    getResult();
        this.emitter.emit('accepted-join-portal-request', msg);
        break;
      }
  ...
  _sendCurrentTextBufferStatesToNewPeer(targetPeerId) {
    ...
    this.emitter.emit('enqueue-message-batchess', msg);
  }
