...
class HostPortalBinding {
  ...
  /**
   * Handle a message from a remote peer (delivered by the portal binding
   * manager)
   */
  ...
  /****************** HANDLE JOINING/LEAVING OF GUEST PEERS *******************/
  ...
  async peerDidJoin(guestPeerId) {
    ...
    await this._sendTextUpdatesToNewPeer(guestPeerId)
    this.emitter.emit('peer-joined', guestPeerId);
  }

  /**
   * Send text updates to a new guest peer
   */
  _sendTextUpdatesToNewPeer(guestPeerId) {
    for (const [id, crdt] of this.crdstById) {
      ...
      for (let i = 0; i < lineArray.length; i++) {
        for (let j = 0; j < lineArray[i].length; j++) {
          ...
          this.portalBindingManager.sendMessageToPeer(guestPeerId, msg)
        }
      }
    }
  }
