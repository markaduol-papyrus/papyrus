  ...
  async handleRemoteMessage(msg) {
    ...
    switch (header.type) {
      ...
      case LOCAL_PEER_ID: {
        ...
        const msgHeader1 = new MessageBuilder().
                           setType(LOCAL_PEER_ID).
                           getResult();
        const msgBody1 = new MessageBuilder().
                         setLocalPeerId(this.localPeerId).
                         getResult();
        const newMsg1 = new MessageBuilder().
                        setHeader(msgHeader1).
                        setBody(msgBody1).
                        getResult();
        this.emitter.emit('delivered-local-peer-id', newMsg1);

        const msgHeader2 = new MessageBuilder().
                           setType(ACCEPTED_PEER_ID).
                           setSenderPeerId(this.localPeerId).
                           getResult();
        const newMsg2 = new MessageBuilder().
                        setHeader(msgHeader2).
                        setBody(msgBody1).
                        getResult();
        this.emitter.emit('created-message-for-server', newMsg2);
        break;
      }
