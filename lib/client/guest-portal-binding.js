...
class GuestPortalBinding {
  ...
  initialise() {
    ...
    const msg = new MessageBuilder().
                ...
                setSenderPeerId(manager.getLocalPeerId()).
                getResult();
  ...
  onJoinPortalRequest(callback) {
    this.emitter.on('join-portal-request', callback);
  }

  onHostAcceptedJoinPortalRequest(callback) {
    this.emitter.on('host-accepted-join-portal-request', callback);
  }

  onLeavePortalRequest(callback) {
    this.emitter.on('leave-portal-request', callback);
  }

  onHostAcceptedLeavePortalRequest(callback) {
    this.emitter.on('host-accepted-leave-portal-request', callback);
  }
  ...
  onAcceptedSiteId(callback) {
    this.emitter.on('accepted-site-id', callback);
  }
    ...
    switch (msg.type) {
      ...
      case SITE_ID_ASSIGNMENT: {
        ...
        const msg = new MessageBuilder().
                    ...
                    setUsername(this.portalBindingManager.getLocalUsername()).
                    setTargetPeerId(msg.senderPeerId).
                    getResult();
        this.emitter.emit('accepted-site-id', msg);
        break;
      }
      ...
      case JOIN_REQUEST_ACCEPTED: {
        const msg = new MessageBuilder().
                    ...
                    getResult();
        this.emitter.emit('host-accepted-join-portal-request', msg);
      }
