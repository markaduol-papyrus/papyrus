...
class GuestPortalBinding {
  ...
  constructor({workspace, notificationManager, portalHostPeerId, localPeerId}) {
    ...
    // Site ID used to initialise the CRDT. This is unique among all peers and
    // represents the order in which the peers joined the portal.
    this.siteId;
  }
  ...
  close() {
    ...
    this.emitter.emit('left-portal', this.portalHostPeerId);
  }
  ...
  onLeftPortal(callback) {
    this.emitter.on('left-portal', callback);
  }

  onJoinedPortal(callback) {
