...
class GuestPortalBinding {
  ...
  async _getOrCreateTextBufferProxyAndCRDT(bufferProxyId) {
    ...
    if (!this.crdtsById.has(bufferProxyId) ||
      ...
      // Create new text editor and text buffer proxy.
      const textEditor = await this.workspace.open();
      log.debug('Text editor object: ', {textEditor: textEditor});
      ...
      // Save the buffer
      // TODO: Move this functionality behind TextBufferProxy?
      if (!this.portalHostUsername) {
        log.warn('Portal host username is undefined.');
      }
      const root = path.resolve();
      log.debug('Path root: ', root);
      const basename = path.basename(bufferProxyId);
      log.debug('Path basename: ', basename);
      const bufferSavePath = path.join(root, this.portalHostUsername, basename);
      log.debug('Buffer save path: ', bufferSavePath);
      ...
      const activeTextEditor = this.workspace.getActiveTextEditor();

      // Check whether current pane is focussed on a TextEditor instance.
      if (activeTextEditor) {
        ...
        const activeEditorUri = activeTextEditor.getBuffer().getUri();
        const targetEditorUri = targetEditor.getBuffer().getUri();

        if (activeEditorUri !== targetEditorUri) {
          const logObj = {
            activeTextEditor: activeTextEditor,
            targetTextEditor: targetEditor,
          };
