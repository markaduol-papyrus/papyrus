...
class PortalBindingManager {
  ...
  async initialise() {
    ...
    await this.peerConnectionLayer.onDidEmitMessage(
      this._hanlePeerConnectionLayerMessage.bind(this)
    );
  }
   ...
   */
  _hanlePeerConnectionLayerMessage(message) {
    const logObj = {message: message};
    log.debug('Handling message from peer connection layer: ', logObj);

    const validMsg = this._validatePeerConnectionLayerMessage(message);
    switch (validMsg.header.source) {
      case PEER:
        this._peerMessageHandler(validMsg.body);
        break;
      case SERVER:
        this._serverMessageHandler(validMsg.body);
        break;
      default:
        log.error('Unknown message source: ', validMsg.source);
  ...
  _addHostPortalEventHandlers() {
    ...
    this.hostPortalBinding.onDeliveredLocalPeerId((msg) => {
