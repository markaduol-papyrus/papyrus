...
class PortalBindingManager {
  ...
  async getOrCreateHostPortalBinding() {
    ...
    this.hostPortalBinding.onPeerJoined(
      this._sayHelloToGuestPeer.bind(this)
    );
  ...
  /**
   * Send the necessary initialisation messages to a new guest peer
   */
  async _sayHelloToGuestPeer(guestPeerId) {
    // Construct a message template
    let msg = {
      type: INSERT,
      portalHostPeerId: this.hostPortalBinding.getPeerId()
    };

    for (const bufferProxyId in this.hostPortalBinding.bufferProxiesById) {
      const [bufferProxy, crdt] =
        this.hostPortalBinding._getTextBufferProxyAndCRDT(bufferProxyId);

      // Add buffer proxy ID to message
      msg.textBufferProxyId =
        this._addUsernameToTextBufferProxyId(bufferProxyId);

      const lineArray = crdt.getLineArray();

      for (let i = 0; i < lineArray.length; i++) {
        for (let j = 0; j < lineArray[i].length; j++) {
          const charObj = lineArray[i][j];
          msg.charObject = charObj;
          this.peerConnectionLayer.sendMessageToPeer(guestPeerId, msg);
        }
      }
    }
